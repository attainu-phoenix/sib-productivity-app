...
{/* 
                ...
                <div className="col-md-1" id="popover">
                    <span id={this.props.eventTitle.replace(/\s/g, "")} className="oi oi-ellipses">

                    </span>
                    <UncontrolledPopover trigger="legacy" placement="bottom" target={this.props.eventTitle.replace(/\s/g, "")}>
                        <PopoverHeader>Options</PopoverHeader>
                        <PopoverBody>
                            <div id="popover_content">
                                <div>
                                    <a className="nav-item nav-link" href="/#" style={CalendarStyles.navLink} data-toggle="modal"
                                     data-target={"#"+this.props.eventDate.replace(/\s/g, "")}><span className="oi oi-pencil">      Edit</span></a>
                                    <a className="nav-item nav-link" href="/#" style={CalendarStyles.navLink} data-toggle="modal" 
                                    data-target={"#1asfdas"+this.props.eventDate.replace(/\s/g, "")}><span className="oi oi-trash">       Delete</span></a>
                                </div>
                            </div>
                        </PopoverBody>
                    </UncontrolledPopover>
                </div>
                <div className="modal fade" id={this.props.eventDate.replace(/\s/g, "")} tabIndex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                    <div className="modal-dialog modal-dialog-centered" role="document">
                        <div className="modal-content">
                            ...
                            <div className="modal-footer">
                                ...
                                <button type="button" className="btn btn-light border" onClick={this.props.onClickEditEvent}>Save</button>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="modal fade" id={"1asfdas"+this.props.eventDate.replace(/\s/g, "")} tabIndex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                    <div className="modal-dialog modal-dialog-centered" role="document">
                        <div className="modal-content">
                            <div className="modal-header" style={CalendarStyles.modalHeadeBackgroundColor}>
                            ...
                            <div className="modal-footer">
                                <button type="button" className="btn btn-secondary border" data-dismiss="modal">Close</button>
                                <button type="button" className="btn btn-light border" onClick={this.props.deleteEvent}>Delete</button>
                            </div>
                        </div>
                    </div>
                </div>
